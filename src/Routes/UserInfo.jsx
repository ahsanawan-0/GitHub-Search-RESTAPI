...
const UserInfo = () => {
  ...
  useEffect(() => {
    // Get data from local storage if it matches the current pathname
    const storedUser = localStorage.getItem(`user_${pathname}`);
    const storedUsers = localStorage.getItem(`users_${pathname}_${type}`);
  ...
  useEffect(() => {
    // Save user and users to local storage whenever they change
    if (user.length > 0) {
      localStorage.setItem(`user_${pathname}`, JSON.stringify(user));
    }
    if (users.length > 0) {
      localStorage.setItem(`users_${pathname}_${type}`, JSON.stringify(users));
    }
  }, [user, users, pathname, type]);
